...
def penal_at_iteration(penal: float, penal_init, loop: int, max_iter: int) -> float:
    """penal tại vòng lặp `loop` (1-indexed) khi bật continuation tuyến tính.

    penal_init=None: trả về `penal` không đổi (hành vi mặc định/cũ).
    penal_init đặt: tăng tuyến tính penal_init -> penal khi loop đi từ 1 -> max_iter
...
def run_simp(params: dict) -> dict:
    ...
    while loop < max_iter:
        ...
        xnew, xPhys = oc_update(x, dc, dv, volfrac, move_t, H, Hs, ft)
