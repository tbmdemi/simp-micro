...
| 5 | Conditional VAE | ✅ Single-shot generation still exploits the surrogate, but **best-of-N + real-FE selection fixes inverse design in practice**: R²(v12, real FE) = **+0.44 to +0.60**, true-auxetic hit rate = **100%** (up from -9.5/0.33 baseline). Generated geometry is also rarely *manufacturable* (connected + periodic-tileable) even when accurate — mitigated at inference (`--require-manufacturable`, large N) at some R² cost, not fully solved by training | Inverse design: target Poisson ratio → generated geometry. A single cVAE sample still cannot be trusted (see surrogate-exploitation findings below), but generating N=30 candidates per target and picking the best by **real FE** (not the surrogate) — the same recipe as the published Deep-DRAM pipeline (Pahlavani et al., *Advanced Materials* 2024) — reliably produces genuinely auxetic geometry. Two training-time remedies (self-play adversarial retraining, surrogate ensembling) were tried first and did **not** fix the single-shot problem within this session's budget; the inference-time fix below does. A separate manufacturability check (connectivity + periodic tiling) found correct Poisson ratio doesn't imply buildable geometry (0-10% joint pass rate, uniform across the property-space, i.e. not a data-coverage gap); training-time fixes gave at most a small improvement, the practical mitigation is inference-time filtering with much larger N. See [Phase 5](#5-conditional-vae-phase-5) below |
...
**Manufacturability (connectivity + periodic tiling), confirmed 2026-07-23 — correct Poisson ratio ≠ buildable geometry:**

`pipeline/phase5_cvae/manufacturability.py` adds two geometric checks that are independent of FE/surrogate accuracy: `check_connectivity()` (no floating disconnected material islands, no strut thinner than a configurable minimum width) and `check_periodicity()` (opposite edges of the 64×64 density field must match — the unit cell is tiled by translation, so a mismatch there means a sudden solid/void jump right at the seam when printed as a lattice; this is a **geometric** check, distinct from the periodic boundary condition `simp/core/pbc.py` applies to the *displacement field* during the FE solve, which says nothing about whether the density image itself tiles cleanly).

Measuring both on `cvae_gamma20.pt`'s `generate()` output (N=200 samples/condition, 3 conditions spanning the target range) found a **globally low, roughly-uniform pass rate**: connectivity-only 7–28%, periodicity-only 3–11%, **both simultaneously only 0–3.5%** — i.e. correct Poisson ratio (which best-of-N already solves, R²=+0.44 to +0.60) says nothing about whether that geometry could actually be built. A property-space coverage sweep (`coverage_eval.py`, grid of 8 targets across the full trained v12 range, `outputs/phase5/self_play/coverage_gamma20.json`) confirmed this is **not a localized dead zone** — the low manufacturability rate (0–10%) shows up uniformly across the whole grid, while property-accuracy hit rate stays 100% everywhere. This matters for the roadmap's active-learning question: since there's no *coverage gap* to fill, targeted data collection (train a real active-learning loop to fill gaps) isn't well-motivated here — the problem is a decoder-behavior issue, not a data-coverage issue.

Two training-time fixes were tried:
1. **Regularize the posterior reconstruction** (`train.py --lambda-tv --lambda-bin --lambda-periodic`, reusing the existing `tv_loss`/`binarization_loss` plus a new `periodicity_loss`, fine-tuned 25 epochs from `cvae_gamma20.pt`) — **no effect** (manufacturability rate unchanged within noise). Root cause: this loss is computed on `recon`, decoded from `z` sampled from the *posterior* (the real encoder, given real training images) — and the real training images (built from symmetric seed shapes: circle/square/reentrant_bowtie/hexagonal) are already close to periodic/connected, so there was almost nothing to fix in that regime. `model.generate()` at inference instead samples `z` from the *prior* (`N(0,1)`, no encoder) — a distribution the posterior-side loss never touches.
2. **Regularize prior-sampled decodes directly** (`train.py --regularize-prior-samples`, new `losses.prior_sample_regularization()`: decode a `z ~ N(0,1)` batch and apply the same tv/bin/periodicity losses to *that* image — the same regime `generate()` actually uses), fine-tuned 40 epochs, stronger weights (`--lambda-periodic 0.3`) — a **small, consistent improvement** (both-pass rate roughly 1.5–2× higher: 0/2, 1/4, 7/10 out of 200 across the 3 test conditions, baseline vs. fine-tuned), without harming property accuracy (selected checkpoint's real-FE R²=-1.11, within the normal range). Not dramatic — checkpoint saved as `outputs/phase5/cvae_manuf_prior.pt`, offered as an optional alternative, **not** swapped in as the new default.

**Practical mitigation: filter for manufacturability at inference, with a much larger N.** `best_of_n_eval.py --require-manufacturable` restricts the candidate pool (ranking + FE verification) to images passing `check_manufacturability()`. At the pool sizes already used for accuracy (N=30, N=300) this actively **hurts** — manufacturable candidates are rare (~1-5%) and their property values aren't evenly distributed, so a small manufacturable pool can lack a good match (measured: N=300/K=5 dropped hit-rate 1.0→0.8 and R² +0.44→**-1.96**). Pushing N to **1500** (still cheap — manufacturability screening needs no FE, only `generate()` + numpy) recovered **hit-rate 1.0, R²=+0.19** (`outputs/phase5/self_play/best_of_n_manuf_n1500.json`) — lower than the unconstrained best-of-N's +0.44–0.60, but positive and 100%-hit, confirming the tradeoff is real but manageable with enough samples:

```bash
python3 pipeline/phase5_cvae/best_of_n_eval.py --n-samples 1500 --k-fe-verify 8 --require-manufacturable
```

**Bottom line (roadmap 6.2/6.3/7.4/7.5):** manufacturability and property-accuracy are in real tension for this generator — use `--require-manufacturable` with large N only when a genuinely buildable design is required (accept a lower R² for that guarantee); otherwise the default property-only best-of-N (R²=+0.44 to +0.60, no manufacturability constraint) remains the better choice when raw Poisson-ratio accuracy is what matters. The coverage map showing no localized dead zones means a real Phase-7-style active-learning data-collection loop (7.1–7.3: add verified candidates back to the dataset, retrain surrogate/generator) is **not well-motivated by the evidence** — this mirrors the already-documented finding that self-play/retraining-based fixes don't move this decoder's behavior; the two levers that actually work are inference-time selection (best-of-N) and, to a smaller degree, prior-sample regularization.
