...
| 5 | Conditional VAE | ✅✅ Surrogate-exploitation đã sửa TẬN GỐC bằng differentiable-physics; đã retrain trên dataset v2 | `cvae_v2_finetuned.pt` (recipe 2-stage: base rồi fine-tune real-physics, 2026-07-25) - đo trên test set v2 (n=300): R²(FE)=**0,995**, hit rate single-shot=**99,7%**, frac manufacturable=0,247. Ngang ngửa `cvae_realphysics.pt` (checkpoint cũ, train trên dataset trước rebuild): R²=0,998, hit rate=98,3%, frac manufacturable=0,280 - đo lại trên CÙNG test set v2 để so sánh công bằng. Xem [Phase 5](#5-conditional-vae-phase-5---thiết-kế-ngược-được-giải-quyết-qua-best-of-n--chọn-lọc-bằng-fe-thực) bên dưới; toàn bộ quá trình thử-sai (gamma-sweep, self-play, ensemble, regularization, đánh giá quy mô lớn) xem [EXPERIMENT_LOG.md](EXPERIMENT_LOG.md) |
...
**2026-07-25 - Rebuild v2 (mở rộng quy mô + 2 bug sửa trong sinh dữ liệu):** dataset production cần lớn hơn (mục tiêu 40-50k ảnh train sau aug) nên sinh thêm một lô mẫu thô mới bằng script song song `analysis/scripts/generate_production_batch.py` (độc lập với `pipeline/phase2_multi_batch/`, dùng `multiprocessing.Pool` để chạy nhiều `run_simp()` song song). Quá trình này phát hiện 2 bug thật trong chính script sinh dữ liệu mới (không phải trong pipeline production Phase 2 gốc):

1. **Range tham số quá rộng cho seed nhạy cảm**: dùng nguyên `ACTIVE_PARAMETERS` (volfrac 0,3-0,7 / void_size_frac 0,1-0,4) cho mọi seed khiến `reentrant_bowtie` chỉ đạt 1,4% yield (so với lịch sử 77,2%). Sửa: `hexagonal`/`reentrant_bowtie` dùng range hẹp đã kiểm chứng (volfrac 0,50-0,58 / void_size_frac 0,28-0,38), `hourglass` giữ range rộng (vốn robust, yield 96,3%).
2. **`beta=0,8` thay vì mặc định production `1.0`** (nghiêm trọng hơn): script mới hardcode `beta=0.8` khi gọi `compute_auxetic_q12_objective(...)`, trong khi `pipeline/phase2_multi_batch/runner.py::build_params_dict()` (pipeline gốc) không bao giờ set `beta` nên luôn dùng mặc định `1.0` của `run_simp`. `beta=0.8` khiến `reentrant_bowtie` suy biến hoàn toàn (kẹt ở ~13/150 iteration, `v12≈0`, 100% rời rạc) dù đã sửa bug #1. Xác nhận qua tái hiện trực tiếp: `beta=0.8` → kẹt 13 iter; `beta=1.0` → chạy đủ 150 iter, `v12=-0,063`. Sau khi sửa cả 2 bug: yield sạch `hourglass`=95,5%, `hexagonal`=96,4%, `reentrant_bowtie`=42,4% (cải thiện nhiều so với 1,4% ban đầu, vẫn thấp hơn lịch sử 77,2% - chênh lệch còn lại chưa điều tra thêm).

Mẫu thô mới (lọc theo `is_connected & osc_score<0,15 & v12<0 & not degenerate`, **không** loại theo `hit_cap`/`converged` - xác nhận lại trong phiên này rằng cờ này không đáng tin làm tiêu chí chất lượng) được gộp với pool sạch cũ (`analysis/scripts/assemble_phase3_v2.py`), chia lại 70/15/15 theo phân tầng seed, tăng cường đối xứng ×6 chỉ trên train. **Kết quả: train=57.216 (đạt mục tiêu 40-50k), val=2.044, test=2.044.** Đã thay thế `outputs/phase3/{train,val,test,dataset_64}.npz` (bộ cũ backup nguyên vẹn tại `outputs/phase3_backup/`, có thể khôi phục).
...
> **2026-07-25 - retrain trên dataset v2** (mục 3, 57.216 mẫu train): `surrogate_v2.pt` đo trên test set v2 (2.044 mẫu) - **v12 R²=0,974** (MAE 0,016), **v21 R²=0,964** (MAE 0,019), **volfrac R²=0,983** (MAE 0,008) - cải thiện thật so với `surrogate_clean.pt` (0,922/0,889/-), phần lớn nhờ quy mô dữ liệu lớn hơn (~2,6× mẫu clean). Export sẵn cho Phase 5 tại `outputs/phase4/surrogate_for_phase5_v2.pt`. Cũng theo quy ước cũ, **không ghi đè** `surrogate_best.pt`/`surrogate_for_phase5.pt` - dùng `--ckpt outputs/phase4/surrogate_v2.pt` / `--surrogate-path outputs/phase4/surrogate_for_phase5_v2.pt` cho công việc mới.
...
**2026-07-25 - retrain trên dataset v2:** dataset production đổi (mục 3) nên retrain lại cVAE để khớp phân phối mới. Train từ đầu với real-physics loss active ngay từ epoch 1 **thất bại** (R²(FE) âm sâu suốt 60 epoch, -2 đến -17) - xác nhận **quy trình 2-stage là bắt buộc**, không thể gộp 1 bước: Stage 1 = base training không/ít real-physics đến hội tụ (`cvae_v2_base.pt`), Stage 2 = fine-tune 35 epoch với `--lambda-real-physics 20.0` từ checkpoint Stage 1 (`cvae_v2_finetuned.pt`). So sánh trên cùng test set v2 (n=300 mỗi checkpoint, `--n-samples 30`):
