...
15. **Physics oracle (loss huấn luyện + oracle đánh giá R²=0,999) dùng CHUNG một engine FE/homogenization nội bộ** (`simp/core/solver.py` + `simp/homogenization/compute.py`), chưa từng đối chiếu với phần mềm FE độc lập bên ngoài codebase (ANSYS/Abaqus/FEniCS/COMSOL). An toàn hơn nhiều so với lỗi surrogate-exploitation trước đây (đây là vật lý thật, đã kiểm chứng finite-difference - không phải mục tiêu học-được có thể bị decoder đánh lừa), nhưng con số R²=0,999 nên đọc là "nhất quán nội bộ, đã kiểm chứng gradient", không phải "đã xác nhận đúng vật lý tuyệt đối". Nếu hướng tới công bố khoa học, nên đối chiếu một tập nhỏ mẫu đại diện (~20-30) bằng công cụ FE độc lập trước khi công bố.

16. **[CHẤP NHẬN 2026-07-29] Seed `hexagonal` có yield thấp hơn lịch sử (~80,5% so với 93,9%) sau khi sửa 2 bug nghiêm trọng (Q-scaling A1 + trạng thái hấp thụ x=0 trong OC), đã xác nhận không có fix đơn giản.** Bug A1 (đã sửa) từng vô tình đóng vai trò regularizer cho `hexagonal` (seed mỏng/khó nhất) suốt lịch sử dự án - sau khi sửa đúng, ngưỡng stiffness `delta=10%` (khớp thiết kế tham chiếu gốc) không đủ mạnh để giữ ổn định như trước cho riêng seed này. Đã thử tăng `delta` (10%→25%) và `beta` (1,0→4,0) - **cả 2 đều phản tác dụng mạnh** (sạch giảm còn 0-10%, do bùng nổ dao động/rời rạc hoặc bộ tối ưu bỏ hẳn mục tiêu auxetic) - xác nhận cấu hình mặc định hiện tại đã là tốt nhất trong các mức đã thử. `hourglass` (98,5%→~85% ở dải volfrac khớp lịch sử) và `reentrant_bowtie` (77,2%→**87,0%, cải thiện thật**) không có vấn đề này. `degenerate=0` (hết sụp cấu trúc vĩnh viễn) ở cả 3 seed (`hourglass` n=100, `hexagonal` n=400, `reentrant_bowtie` n=100). Chưa rebuild dataset 57k mẫu sản xuất - dataset hiện có sinh bằng code cũ (trước các fix này), không bị ảnh hưởng. Chi tiết đầy đủ: [EXPERIMENT_LOG.md](EXPERIMENT_LOG.md).
...
15. **The physics oracle (training loss + evaluation oracle, R²=0.999) uses the SAME internal FE/homogenization engine** (`simp/core/solver.py` + `simp/homogenization/compute.py`), never cross-checked against an FE tool external to this codebase (ANSYS/Abaqus/FEniCS/COMSOL). Much safer than the earlier surrogate-exploitation failure mode (this is real physics, finite-difference verified - not a learnable target a decoder could game), but the R²=0.999 figure should be read as "internally consistent, gradient-verified", not "confirmed correct against absolute ground truth". Before any scientific publication, cross-check a small representative sample (~20-30) against an independent FE tool.
