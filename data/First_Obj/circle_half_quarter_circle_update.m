function circle_half_quarter_circle_update(nelx, nely, volfrac, penal, rmin, ft, nu, beta, ...
...
% =========================================================================
% PER-PATTERN 2: SEED MASK — canonical circle_half_quarter_circle
% Native legacy: center r=nelx/9 (~11), half/quarter r=nelx/18 (~5.5).
% vsf scaling: center_r = vsf * nelx / 3, half/quarter_r = vsf * nelx / 6.
% At vsf=0.33 -> center_r=11, half_r=5.5 (matches legacy at vsf=0.33).
[I, J] = meshgrid(1:nelx, 1:nely);
center_x = nelx / 2; center_y = nely / 2;
center_r  = void_size_frac * nelx / 3;
half_r    = void_size_frac * nelx / 6;
quarter_r = void_size_frac * nelx / 6;

seed_mask = (sqrt((I - center_x).^2 + (J - center_y).^2) <= center_r);

edge_pos = [center_x, 1; center_x, nely; 1, center_y; nelx, center_y];
for pos = 1:4
    cx = edge_pos(pos,1); cy = edge_pos(pos,2);
    m = (sqrt((I - cx).^2 + (J - cy).^2) <= half_r);
    if pos == 1, m = m & (J >= cy);
    elseif pos == 2, m = m & (J <= cy);
    elseif pos == 3, m = m & (I >= cx);
    elseif pos == 4, m = m & (I <= cx);
    end
    seed_mask = seed_mask | m;
end

corner_pos = [1, 1; nelx, 1; 1, nely; nelx, nely];
for pos = 1:4
    cx = corner_pos(pos,1); cy = corner_pos(pos,2);
    m = (sqrt((I - cx).^2 + (J - cy).^2) <= quarter_r);
    if pos == 1, m = m & (I >= cx) & (J >= cy);
    elseif pos == 2, m = m & (I <= cx) & (J >= cy);
    elseif pos == 3, m = m & (I >= cx) & (J <= cy);
    elseif pos == 4, m = m & (I <= cx) & (J <= cy);
    end
    seed_mask = seed_mask | m;
end
% END PER-PATTERN 2
