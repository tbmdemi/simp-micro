...
function circle_half_quarter_circle(nelx, nely, volfrac, penal, rmin, ft)
...
% Parameters
center_circle_radius = floor(min(nelx, nely) / 9); % Radius of the center circle
half_circle_radius = floor(min(nelx, nely) / 18); % Radius of the half-circle voids
quarter_circle_radius = floor(min(nelx, nely) / 18); % Radius of the quarter-circle voids
center_x = nelx / 2; % Center of the design domain (x-direction)
center_y = nely / 2; % Center of the design domain (y-direction)

% Create the central circular void
for i = 1:nelx
    for j = 1:nely
        if sqrt((i - center_x)^2 + (j - center_y)^2) <= center_circle_radius
            x(j, i) = volfrac / 2;
        end
    end
end

% Create half-circle voids at the edges with reversed directions
edge_positions = [
    center_x, 1; % Top edge
...
for pos = 1:size(edge_positions, 1)
    cx = edge_positions(pos, 1);
    cy = edge_positions(pos, 2);

    for i = 1:nelx
        for j = 1:nely
            if sqrt((i - cx)^2 + (j - cy)^2) <= half_circle_radius
                if (pos == 1 && j >= cy) || (pos == 2 && j <= cy) || ...
                   (pos == 3 && i >= cx) || (pos == 4 && i <= cx)
                    x(j, i) = volfrac / 2;
                end
            end
        end
...
% Create quarter-circle voids at the corners with reversed directions
corner_positions = [
    1, 1; % Top-left
    nelx, 1; % Top-right
    1, nely; % Bottom-left
    nelx, nely % Bottom-right
];

for pos = 1:size(corner_positions, 1)
    cx = corner_positions(pos, 1);
    cy = corner_positions(pos, 2);

    for i = 1:nelx
        for j = 1:nely
            if sqrt((i - cx)^2 + (j - cy)^2) <= quarter_circle_radius
                if (pos == 1 && i >= cx && j >= cy) || ...
                   (pos == 2 && i <= cx && j >= cy) || ...
                   (pos == 3 && i >= cx && j <= cy) || ...
                   (pos == 4 && i <= cx && j <= cy)
