...
function topK_Four_circle(nelx, nely, volfrac, penal, rmin, ft) %%please set a low rmin and high penal
...
%% Create folder to save the interations
folderName = ['File_', datestr(now, 'yyyy-mm-dd_HHMMSS')];
...
%% Initialize xPhys with uniform material distribution
x = repmat(volfrac, nely, nelx);  % Set entire design domain to uniform density

%% Define Four Circles with Reduced Density or Voids
radius = 10;  % Radius of circles
offset = 10;  % Offset from the edges
centers = [
    offset + radius, offset + radius;  % Bottom-left
    nelx - offset - radius, offset + radius;  % Bottom-right
    offset + radius, nely - offset - radius;  % Top-left
    nelx - offset - radius, nely - offset - radius  % Top-right
];
for c = 1:size(centers, 1)
    [X, Y] = meshgrid(1:nelx, 1:nely);  % Create a grid of x, y coordinates
    circleMask = (X - centers(c, 1)).^2 + (Y - centers(c, 2)).^2 <= radius^2;
    x(circleMask) = volfrac/2;  % Set the circle areas to zero or a reduced density
end
xPhys = x;  % Initialize x for optimization to match xPhys
...
xi = 0.1;  % Constraint parameter, can be adjusted as needed
...
while (change > 0.01)
    ...
    c = Q(1,2) - (0.8^loop) * (Q(1,1) + Q(2,2)); %% objective function with the stiffness matrix Q(1, 2): shear stiffness, Q(1,1) and Q(2,2): main direction
    ...
    imagesc(1 - xPhys);  % This scales color by effective E
