...
function nine_circle_New_obj(nelx, nely, volfrac, penal, rmin, ft)
...
x = repmat(volfrac, nely, nelx); % Initialize with the material fraction (volfrac)

% Parameters for circular voids
num_circles_x = 3; % Number of circles in the x-direction
num_circles_y = 3; % Number of circles in the y-direction
circle_radius = floor(min(nelx, nely) / 12); % Radius of each circular void
margin_x = nelx / (num_circles_x + 1); % Margin from edges and between circles in x-direction
margin_y = nely / (num_circles_y + 1); % Margin from edges and between circles in y-direction

% Loop to create circular voids
for i = 1:num_circles_x
    for j = 1:num_circles_y
        % Calculate the center of the current circular void
        center_x = round(i * margin_x);
        center_y = round(j * margin_y);

        % Loop over the design space to remove material within the circle radius
        for row = 1:nely
            for col = 1:nelx
                % Check if the point is within the circular void radius
                if sqrt((row - center_y)^2 + (col - center_x)^2) <= circle_radius
                    x(row, col) = volfrac/2; % Assign 0 for void
                end
            end
        end
    end
end
xPhys = x; % Final design space
