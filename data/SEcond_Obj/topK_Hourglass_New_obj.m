...
function topK_Hourglass_New_obj(nelx, nely, volfrac, penal, rmin, ft)
%% nelx, nely: number of elements along x, y
%% MATERIAL PROPERTIES
E0 = 1;
Emin = 1e-9;
nu = 0.3;
rho0 = 7850;  % Adjust density if necessary
...
%% INITIALIZE DESIGN WITH RE-ENTRANT (HOURGLASS) VOID
...
%% INITIALIZE DESIGN WITH RE-ENTRANT (HOURGLASS) VOID WITH ANGLED TAPER TOWARDS WAIST
x = repmat(volfrac, nely, nelx);  % Initialize with uniform density
hourglass_width = nelx / 14;  % Adjust the width of the hourglass void at the waist
hourglass_height = nely / 5;  % Adjust the height of the hourglass void
center_x = nelx / 2;  % X-coordinate of the center
center_y = nely / 2;  % Y-coordinate of the center
taper_angle_deg = 50;  % Desired taper angle from top/bottom to waist, in degrees
taper_slope = tan(deg2rad(taper_angle_deg));  % Slope based on taper angle

for i = 1:nelx
    for j = 1:nely
        % Calculate vertical distance from the waist center
        vertical_dist = abs(j - center_y);
        
        % Define the width of the hourglass shape based on vertical distance
        taper_width = hourglass_width + taper_slope * vertical_dist;
        
        % Check if the point is within the hourglass shape with angled taper
        if abs(i - center_x) < taper_width && vertical_dist < hourglass_height
            x(j,i) = volfrac / 2;  % Lower density for the void region
        end
    end
end
...
change = 1;
loop = 0;
tolerance = 0.04;  % 5% tolerance for stopping criterion
...
while (change > 0.01)
    loop = loop+1;
    %% FE-ANALYSIS
    sK = reshape(KE(:)*(Emin + (rho0 * xPhys(:)'.^penal * (E0 - Emin))),64*nelx*nely,1);
    ...
    %% Poisson's ratios remain the same
