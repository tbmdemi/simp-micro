...
function topK_square_New_obj(nelx, nely, volfrac, penal, rmin, ft)
...
hole_size_x = floor(nelx / 3); % Define the size of the square hole (adjust as needed)
hole_size_y = floor(nely / 3);

% Define the range for the square hole
hole_start_x = floor(nelx / 2 - hole_size_x / 2);
hole_end_x = floor(nelx / 2 + hole_size_x / 2);
hole_start_y = floor(nely / 2 - hole_size_y / 2);
hole_end_y = floor(nely / 2 + hole_size_y / 2);

% Set the square hole in the middle
x(hole_start_y:hole_end_y, hole_start_x:hole_end_x) = volfrac / 2;
xPhys = x;
...
while (change > 0.01)
    ...
    if loop < 10 % Control for the first 10 iterations
    ...
    penalty = 1e6;  % Penalty factor for constraint violations
    if Q(1,1) < delta
