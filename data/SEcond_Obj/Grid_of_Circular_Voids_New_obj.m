...
function Grid_of_Circular_Voids_New_obj(nelx, nely, volfrac, penal, rmin, ft)
...
% Parameters
center_circle_radius = floor(min(nelx, nely) / 8); % Radius of the center circle
half_circle_radius = floor(min(nelx, nely) / 12); % Radius of the half-circle voids
quarter_circle_radius = floor(min(nelx, nely) / 12); % Radius of the quarter-circle voids
...
% Create two half-circle voids at the 1/3 and 2/3 positions along each edge
for edge = 1:4
    if edge == 1 % Top edge
        positions = [
            nelx / 3, 1;
    ...
    elseif edge == 2 % Bottom edge
        positions = [
            nelx / 3, nely;
            2 * nelx / 3, nely
        ];
    elseif edge == 3 % Left edge
        positions = [
            1, nely / 3;
            1, 2 * nely / 3
        ];
    elseif edge == 4 % Right edge
        positions = [
            nelx, nely / 3;
            nelx, 2 * nely / 3
        ];
...
% Create quarter-circle voids at the corners
corner_positions = [
    1, 1; % Top-left
    nelx, 1; % Top-right
    1, nely; % Bottom-left
...
while (change > 0.01)
   ...
   %% Calculate delta and apply constraints
    ...
    if loop < 30 % Control for the first 20 iterations
