...
function topK_Four_circle_New_obj(nelx, nely, volfrac, penal, rmin, ft) %%please set a low rmin and high penal
...
%% PREPARE FILTER (Increase Filter Radius for Smoothness)
...
centers = [
    ...
    nelx - offset - radius, offset + radius;
    offset + radius, nely - offset - radius;
    nelx - offset - radius, nely - offset - radius
];
for c = 1:size(centers, 1)
    [X, Y] = meshgrid(1:nelx, 1:nely);
    circleMask = (X - centers(c, 1)).^2 + (Y - centers(c, 2)).^2 <= radius^2;
    x(circleMask) = volfrac/2;
end
...
while (change > 0.01)
    ...
    if loop < 10 % Control for the first 20 iterations
        scale_factor = 1 - 0.02 * loop;
        c = c - scale_factor * (Q(1,1) + Q(2,2));
    end

    penalty = 1e2;  % Reduced Penalty factor
    if Q(1,1) < delta
        c = c + penalty * (delta - Q(1,1))^2;  
      fprintf('Iteration: %d - Horizontal stiffness penalty applied!\n', loop);
    end
    if Q(2,2) < delta
        c = c + penalty * (delta - Q(2,2))^2;  
    fprintf('Iteration: %d - Verticle stiffness penalty applied!\n', loop);
    end
    ...
    l1 = 0; l2 = 1e9; move = 0.03;  % Reduced move limit
    while (l2-l1 > 1e-9)
        lmid = 0.5*(l2+l1);
