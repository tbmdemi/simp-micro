...
### 2.6. `simp/core/oc.py` - Cập nhật Optimality Criteria
...
### 2.7. `simp/core/filter.py` - Bộ lọc mật độ hình nón
...
| **Filter type 1 (sensitivity)** | dc_filtered = H(x·dc) / (Hs · x) - **được chấp nhận, nhưng cần đối chiếu với mô hình nhạy cảm gradient adjoint reconstruction gần đây để tăng tính chính xác.** |
...
### 2.8. `simp/core/convergence.py` - Phát hiện hội tụ
...
### 2.9. `simp/materials/isotropic.py` - Ma trận độ cứng phần tử
...
### 2.10. `simp/seeds/` - 10 mẫu seed khởi tạo
...
### 2.11. `simp/runner.py` - Vòng lặp chính điều phối
...
### 2.12. `pipeline/phase1_screening.py` - LHS Screening
...
### 2.13. `pipeline/phase2_tuning.py` - Tuning tham số
...
### 2.14. `analysis/` - Phân tích hậu kỳ
...
### 🔴 Critical 1: Sai `max` thay vì `min` trong `scripts/aggregate_correlations.py`
...
### 🟡 Warning 4: CSV column naming inconsistency
